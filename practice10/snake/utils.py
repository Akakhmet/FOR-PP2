...
def draw_cell(surface, col, row, color, margin=1, hud_offset=True):
    """Draw a filled rectangle for one grid cell with optional margin."""
...
def draw_hud(surface, score, level, fps):
    ...
    speed_surf = font.render(f"SPEED: {fps} FPS", True, GRAY)
...
def draw_overlay(surface, title, subtitle=""):
    ...
    from settings import BLACK, GREEN, WHITE
...
def draw_background_grid(surface):
    """Draw subtle background dots in the playable area."""
